...
df_train = pd.read_csv('split/train_full_PBDD.csv')
df_eval = pd.read_csv('split/val_full_PBDD.csv')
df_test = pd.read_csv('split/test_full.csv')
train_file = f'split/train_full.csv'
eval_file = f'split/val_full.csv'
...
model.train_model(df_train[['smiles', 'E_gap']], output_dir=r'train/model_config',
                  eval_df=df_eval)
predict_data(model, df_train, 'split/PBDD-train')
predict_data(model, df_eval, 'split/PBDD-valid')
predict_data(model, df_test, 'split/MpPD-test')
print('finish')
