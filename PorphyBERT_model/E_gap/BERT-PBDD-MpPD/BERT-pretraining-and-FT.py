...
df = pd.read_csv('/home/dell/PycharmProjects/zx-all-model/PBDD.csv')
df_train, df_test = train_test_split(df[['smiles', 'E_gap']], train_size=0.8, random_state=42)
if not os.path.exists('split'):
    os.mkdir(os.getcwd() + '/split')
df_train.to_csv('split/train_PBDD.csv', index_label=False, index=False)
df_test.to_csv('split/test_PBDD.csv', index_label=False, index=False)
train_file = f'split/train_PBDD.csv'
eval_file = f'split/test_PBDD.csv'
...
model.train_model(df_train[['smiles', 'E_gap']], output_dir=r'train/model_config',
                  eval_df=df_eval)
